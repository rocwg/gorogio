...
### 13.3 重点
- 可以控制 stdin/stdout/stderr。
- 适合需要流式处理的场景。

### 13.4 常见坑
- 命令输出没重定向。
- 不理解 `Run`、`Output`、`CombinedOutput` 的区别。
...
### 15.4 常见坑
- 不知道 `defer` 不会在 `os.Exit` 后执行。
- 在 goroutine 里随意 `Exit`。
